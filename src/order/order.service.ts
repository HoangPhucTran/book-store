...
export class OrderService {
  ...
  async edit(id: string, orderDto: OrderEditRequestDto): Promise<Order> {
    try {
      ...
      if (orderDto?.status !== undefined) order.orderStatus = orderDto.status;
      ...
      if (orderDto?.item !== undefined) {
        ...
        if (!orderItems) throw new NotFoundException('Order Item not found');
        ...
        for (const dtoItem of orderDto.item) {
          ...
          const orderItem = orderItems.find((i) => i.bookId === dtoItem.bookId);
